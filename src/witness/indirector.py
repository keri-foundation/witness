...
class Indirector(doing.DoDoer):
    ...
    def __init__(self, hab, client, direct=True, doers=None, **kwa):
        ...
        Parameters:
            ...
            direct is Boolean, True means direct mode process cured receipts
                               False means indirect mode don't process cued receipts

        """
        self.hab = hab
        self.client = client  # use client for both rx and tx
        self.direct = True if direct else False
        self.kevery = eventing.Kevery(db=self.hab.db, lax=False, local=False, cloned=not self.direct, direct=self.direct)
        self.parser = parsing.Parser(ims=self.client.rxbs, framed=True, kvy=self.kevery)
        doers = doers if doers is not None else []
        doers.extend([doing.doify(self.msg_do), doing.doify(self.escrow_do)])
        if self.direct:
            doers.extend([doing.doify(self.cue_do)])
    ...
    def cue_do(self, tymth=None, tock=0.0):
        ...
        while True:
            for msg in self.hab.processCuesIter(self.kevery.cues):
                self.send_message(msg, label='chit or receipt')
    ...
    def send_message(self, msg, label=''):
        """
