...
parser.add_argument('--salt', action='store', required=False, default=None)
...
def run(
    ...
    if configFile is not None:
        if configDir is not None:
            ...
            cf = configing.Configer(name=configFile, temp=False, reopen=True, clear=False)
    elif configDir is not None:
        cf = configing.Configer(headDirPath=configDir, temp=False, reopen=True, clear=False)

    if aeid is None:
        print(name, base, bran, cf, salt)
        hby = habbing.Habery(name=name, base=base, bran=bran, cf=cf, salt=salt)
    else:
        print("hihihi")
        print(name, base, bran, cf)
    ...
    doers.extend(
        witness.setup(
            alias=alias, hby=hby, tcpPort=tcp, httpPort=http, keypath=keypath, certpath=certpath, cafilepath=cafilepath
        )
    )
