...
class ReceiptEnd(doing.DoDoer):
    ...
    Most times a witness will be able to return its receipt for an event in-band.  This API
    ...
    def __init__(self, hab, inbound=None, outbound=None, aids=None):
        ...
        self.aids = aids
        self.receipts = set()
        self.psr = parsing.Parser(framed=True, kvy=self.hab.kvy)

        super(ReceiptEnd, self).__init__(doers=[doing.doify(self.intercept_do)])
        ...
        if pre in self.hab.kevers:
            ...
            if self.hab.pre not in wits:
                raise falcon.HTTPBadRequest(
                    description=f'{self.hab.pre} is not a valid witness for {pre} event at ' f'{serder.sn}: wits={wits}'
                )
    ...
    def on_get(self, req, rep):
        ...
        if sn is not None:
            said = self.hab.db.getKeLast(key=dbing.snKey(pre=preb, sn=sn))
        ...
        if self.hab.pre not in wits:
            raise falcon.HTTPBadRequest(
                description=f'{self.hab.pre} is not a valid witness for {pre} event at ' f'{serder.sn}, {wits}'
            )
        ...
        if wigs := self.hab.db.getWigs(key=dgkey):
            rct.extend(coring.Counter(code=coring.CtrDex.WitnessIdxSigs, count=len(wigs)).qb64b)
            for wig in wigs:
                rct.extend(wig)
    ...
    def intercept_do(self, tymth=None, tock=0.0):
        """
        ...
        while True:
            while self.inbound:  # iteratively process each cue in cues
                ...
                cue_kin = cue['kin']  # type or kind of cue

                if cue_kin in ('receipt',):  # cue to receipt a received event from other pre
